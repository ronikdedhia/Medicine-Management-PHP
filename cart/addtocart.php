...
if (isset($_POST['updateid'])){  
 // require_once ("include/initialize.php");
    ...
    $max=count($_SESSION['medicine_cart']);
    for($i=0;$i<$max;$i++){

      $pid=$_SESSION['medicine_cart'][$i]['productid'];
     ...
     if($row['PRICE']< 50){
          ...
          if($qty>=$fixedqty  && $qty<=999){
            ...
            $_SESSION['medicine_cart'][$i]['qty']=$qty;
            $_SESSION['medicine_cart'][$i]['price']=$price;
          }
        }else{
          if($qty>0  && $qty<=999){
...
<!-- <div class="container"> -->
  ...
  <?php check_message(); ?>
      ...
      <div class="table-responsive">
       <div  class="medicine_scroll_carttwo" >
          <table  class="table medicine-table" id="table" >
             <thead>
               <tr>
                  <th width="5px">#</th>
                  <th>Product</th>
         ...
         <tbody > 
               ...
               <?php 
              if (!empty($_SESSION['medicine_cart'])){ 

                echo '<script>totalprice();</script>';
                    $count_cart = count($_SESSION['medicine_cart']);
                    for ($i=0; $i < $count_cart  ; $i++) { 
                    $query = "SELECT * FROM `tblproducts` p , `tblcategory` c 
                      WHERE  p.`CATEGORYID`=c.`CATEGORYID` and PRODUCTID='".$_SESSION['medicine_cart'][$i]['productid']."'";
                      $mydb->setQuery($query);
            ...
            ?>
                      <tr>
                        ...
                        <td><input type="NUMBER" data-id="<?php echo $result->PRODUCTID;  ?>" class="cusqty" name="QTY<?php echo $result->PRODUCTID;  ?>" id="QTY<?php echo $result->PRODUCTID; ?>"  value="<?php echo $_SESSION['medicine_cart'][$i]['qty'] ?>"/>
                        <td>&#8369 <output id="Osubtot<?php echo $result->PRODUCTID ?>"><?php echo   $_SESSION['medicine_cart'][$i]['price'] ; ?></output>

                        <!-- hidden textbox -->
                        <input type="hidden" name="TOT<?php echo $result->PRODUCTID;  ?>" id="TOT<?php echo $result->PRODUCTID; ?>"  value="<?php echo $_SESSION['medicine_cart'][$i]['price'] ?>"/></td>
                        <input type="hidden" name="PRICE<?php echo $result->PRODUCTID;  ?>" id="PRICE<?php echo $result->PRODUCTID; ?>"  value="<?php echo $_SESSION['medicine_cart'][$i]['price'] ?>"/></td>
                     ...
                     <strong><h1 align="right">Total Price : &#8369 <span style="margin-right:3%"  id="sum">0.00</span></h1></strong></td>
                        ...
                        <?php    
                             ...
                             echo '<a href="index.php?page=2" class="btn btn-default"><span class="glyphicon glyphicon-arrow-left"></span>&nbsp;<strong>Add Another Products</strong></a>';
                                $countcart =isset($_SESSION['medicine_cart'])? count($_SESSION['medicine_cart']) : "0";
                                 if ($countcart > 0){
                
                                if (isset($_SESSION['cus_id'])){  
                             
                                  echo '<button type="submit"  name="proceed" id="proceed" class="btn btn_medicine pull-right"   ><strong>Proceed And Checkout</strong><span class="glyphicon glyphicon-chevron-right"></button>';
                               
                                 }else{
                                   echo   '<button type="button" class="btn btn_medicine pull-right" data-target="#myModal" data-toggle="modal" name="btnorder"><strong>Proceed And Checkout</strong><span class="glyphicon glyphicon-chevron-right"></button>';
                                } 
                              }
                                  echo '</form>'; 
                ...
                <div class="panel-head" style="margin-left:3%"><h3>address</h3></div>
                 ...
                 <!--  <div class="form-group">
                  ...
                  <div class="modal-footer"><p align="left">&copy; Fix N Mix Ordering System</p>
                    <button class="btn btn-default" data-dismiss="modal" type=
                    "button">Close</button> 
                    <button type="submit"  name="savecustomer"   class="submit btn btn_medicine" >Sign Up</button> 
